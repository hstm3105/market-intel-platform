...
const mocks = vi.hoisted(() => ({ active: vi.fn(), overview: vi.fn(), updateRetention: vi.fn(), runRetention: vi.fn(), audit: vi.fn(), getScan: vi.fn() }));
...
vi.mock("./governance", () => ({ getGovernanceOverview: mocks.overview, recordAuditEvent: mocks.audit, runRetention: mocks.runRetention, updateRetentionPolicy: mocks.updateRetention }));
...
describe("marketIntel governance controls", () => {
  beforeEach(() => { vi.clearAllMocks(); mocks.active.mockResolvedValue(activeFor("admin")); mocks.overview.mockResolvedValue({ policy: {}, events: [], runs: [] }); mocks.updateRetention.mockResolvedValue({ organizationId: "org-private" }); mocks.runRetention.mockResolvedValue({ organizationId: "org-private", action: "preview", status: "completed" }); mocks.getScan.mockResolvedValue({ scan: { id: "scan-1" } }); });
  ...
  it("allows only administrators to preview or execute retention processing", async () => {
    await marketIntelRouter.createCaller(context()).governance.runRetention({ action: "preview" });
    expect(mocks.runRetention).toHaveBeenCalledWith("org-private", 5, "preview");
    mocks.active.mockResolvedValue(activeFor("viewer"));
    await expect(marketIntelRouter.createCaller(context()).governance.runRetention({ action: "execute" })).rejects.toMatchObject({ code: "FORBIDDEN" });
  });
